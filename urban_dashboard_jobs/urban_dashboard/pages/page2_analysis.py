...
def metric_card(label, value, unit="", colour="#4CAF50"):
    st.markdown(
...
def render():
    ...
    with col_sel:
        ...
        city1 = st.selectbox("Primary city", cities_list, index=default_idx)
    ...
    with left:
        ...
        with mc1:
            metric_card("🌿 NDVI", f"{m1['NDVI']:.3f}", colour=ndvi_color(m1["NDVI"]))
            metric_card("🏙️ Urbanization", f"{m1['Urbanization Score']:.0f}", "/100", "#FF9800")
            metric_card("🌡️ Land Temp", f"{m1['Land Surface Temp']:.1f}", "°C", "#FF5722")
            metric_card("☔ Rainfall", f"{m1['Rainfall']:.0f}", "mm", "#2196F3")
        with mc2:
            metric_card("🏗️ NDBI", f"{m1['NDBI']:.3f}", colour="#795548")
            metric_card("🔥 Heat Stress", f"{m1['Heat Stress (0-10)']:.1f}", "/10", "#FF5722")
            metric_card("🌊 Flood Risk", f"{m1['Flood Risk (0-10)']:.1f}", "/10", "#1565C0")
            metric_card("🛡️ Climate Resilience", f"{m1['Climate Resilience']:.0f}", "/100", "#009688")

        st.markdown(
            f"""<div class="metric-card" style="border-left-color:{aq_col}">
        ...
        st.markdown(
            f"""<div class="metric-card" style="border-left-color:#9C27B0">
