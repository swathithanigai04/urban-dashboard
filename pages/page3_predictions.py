...
def load_models():
    ...
    if not all(os.path.exists(p) for p in paths):
        ...
        train_script = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "models", "train_models.py")
        subprocess.run([sys.executable, train_script], check=True)

    MODELS_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + "/models"
    with open(os.path.join(MODELS_DIR, "xgb_livability.pkl"), "rb") as f: xgb_m   = pickle.load(f)
    with open(os.path.join(MODELS_DIR, "rf_aqi.pkl"),         "rb") as f: rf_aqi  = pickle.load(f)
    with open(os.path.join(MODELS_DIR, "rf_urban.pkl"),       "rb") as f: rf_urb  = pickle.load(f)
...
def simulate_unet_change(ndvi_old, ndvi_new, ndbi_old, ndbi_new, size=64):
    ...
    def make_mask(ndvi, ndbi, size):
        ...
        for i in range(size):
            for j in range(size):
                r = np.random.random()
                veg_prob   = np.clip(ndvi * 1.5, 0, 0.9)
                built_prob = np.clip((ndbi + 0.3) * 2.0, 0, 0.9)
...
# ── Main render ───────────────────────────────────────────────────────────────
def render():
    st.title("Vegetation Prediction")
    ...
    city = st.selectbox("Select city", cities_list, index=default_idx, key=f"sel_p3_{session_city}")
    ...
    with tab1:
        ...
        ndvi_old = min(0.85, float(row["NDVI"]) + 0.12)
        ndbi_old = max(-0.29, float(row["NDBI"]) - 0.06)
        ndvi_new = float(row["NDVI"])
        ndbi_new = float(row["NDBI"])
        ...
        with c1:
            ...
            st.image(past_rgb, caption="Segmented land cover", use_container_width=True)
        with c2:
            st.markdown("**Current**")
            st.image(current_rgb, caption="Segmented land cover", use_container_width=True)
        with c3:
            st.markdown("**Change Map**")
            diff_display = (diff * 80).astype(np.uint8)
            st.image(diff_display, caption="Changed pixels", use_container_width=True)

        # Stats
        total = past.size
        def pct(mask, cls): return round((mask == cls).sum() / total * 100, 1)

        # Only 3 meaningful categories (water excluded — not observed in simulation)
        categories = ["Built-up", "Vegetation", "Bare soil"]
        cls_ids    = [0, 1, 2]

        past_vals    = [pct(past,    c) for c in cls_ids]
        current_vals = [pct(current, c) for c in cls_ids]

        # Future: assume 10-yr trend continues (built-up grows, vegetation shrinks)
        future_vals = [
            round(min(100, current_vals[0] + (current_vals[0] - past_vals[0])), 1),
            round(max(0,   current_vals[1] + (current_vals[1] - past_vals[1])), 1),
            round(max(0,   current_vals[2] + (current_vals[2] - past_vals[2])), 1),
        ]
        ...
        stat_df = pd.DataFrame({
            ...
            "Future (predicted) (%)": future_vals,
        })
        stat_df["Change (Past→Now)"] = [round(c - p, 1) for p, c in zip(past_vals, current_vals)]

        st.dataframe(stat_df, use_container_width=True, hide_index=True)

        # Layman-friendly graph: all three time points
        fig_lc = go.Figure()
        fig_lc.add_trace(go.Bar(name='10 Years Ago', x=categories, y=past_vals,    marker_color='#607D8B'))
        fig_lc.add_trace(go.Bar(name='Today',        x=categories, y=current_vals, marker_color='#4CAF50'))
        fig_lc.add_trace(go.Bar(name='Future (predicted)', x=categories, y=future_vals, marker_color='#FF9800',
                                marker=dict(opacity=0.8, line=dict(color='#FFF176', width=1.5))))
        ...
        st.markdown(
            ...
            "- 🏗️ **Built-up:** Represents roads, buildings, and concrete. An increase here means the city is rapidly expanding and developing.\n"
            "- 🌿 **Vegetation:** Represents trees, parks, and natural green spaces. A decrease in vegetation often leads to hotter city temperatures and higher pollution.\n"
            "- 🟤 **Bare soil:** Represents empty ground. Changes here usually indicate new construction or clearance.\n"
            "- 🟠 **Future (predicted):** Extrapolates the current 10-year trend forward — if the city continues at this pace, this is what land cover will look like.\n\n"
            "*By seeing how fast green spaces are turning into concrete, we can predict future environmental health and the city's livability trajectory.*"
        )
    ...
    with tab3:
        ...
        r4.metric("Vegetation", f"{max(0, wi_ndvi * 100):.0f}%")
    ...
    with tab4:
        ...
        report_items = [
            ("Predicted Livability",  lv_pred,                "Livability",   "/100"),
            ("Predicted AQI",         aqi_pred,               "AQI",          "1=best, 5=worst"),
            ("Predicted Urbanization", urb_pred,              "Urbanization", "(Low/Med/High)"),
            ("Climate Resilience",    m["Climate Resilience"],"Climate Resilience", "/100"),
        ]

        cols = st.columns(4)
        for i, (label, val, metric_key, unit) in enumerate(report_items):
            if metric_key == "Urbanization":
                g, gc = URBAN_LABELS[val], URBAN_COLORS[val]
            else:
...
=== PREDICTED SCENARIO (What-If Simulator) ===
Input NDVI:          {wi_ndvi:.2f}
Input NDBI:          {wi_ndbi:.2f}
Input Rainfall:      {wi_rain} mm
Input Temp:          {wi_temp:.1f} °C
Predicted Livability: {lv_pred:.2f} / 100
Predicted AQI:       {aqi_pred} / 5
Predicted Urbanization: {URBAN_LABELS[urb_pred]}
"""
        st.download_button(
