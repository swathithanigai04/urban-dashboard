...
# ── Load dataset once ────────────────────────────────────────────────────────
_DATA_PATH = os.path.join(os.path.dirname(__file__), "..", "data", "cities.csv")

def load_data():
    df = pd.read_csv(os.path.abspath(_DATA_PATH))
    df.columns = [c.strip() for c in df.columns]
    
    import streamlit as st
    if "dataset_row" in st.session_state and st.session_state["dataset_row"] is not None:
        new_row = st.session_state["dataset_row"]
        try:
            city_name = new_row["city"]
        except Exception:
            city_name = None
            
        if city_name and city_name not in df["city"].values:
            new_df = pd.DataFrame([new_row.to_dict()])
            df = pd.concat([df, new_df], ignore_index=True)
            
            # Ensure proper dtypes for numerical columns
            numeric_cols = ["NDVI", "NDBI", "Rainfall", "Temperature", "latitude", "longitude", "AQI", "Livability"]
